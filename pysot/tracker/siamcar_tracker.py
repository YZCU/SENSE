...
class SiamCARTracker(SiameseTracker):
    ...
    def com_mx(self, a):
        ...
        return np.max(a) * (np.square(np.max(a) - np.min(a)) / np.mean(np.square(a - np.min(a))))
    ...
    def track(self, img, hp, kfpb):
        ...
        if amx > T:
            ...
            cxy = [(x + w / 2, y + h / 2) for x, y, w, h in kfpb]
            pcxy = self.kf(cxy) 
            self.center_pos = pcxy
            wh = [(w, h) for x, y, w, h in kfpb]
            pwh = self.kf(wh)
            w, h = pwh

            # generic
            w1, h1 = pwh * 1.11
            self.size = np.array([w1, h1])
            bbox = [pcxy[0] - w / 2, pcxy[1] - h / 2, w1, h1]
