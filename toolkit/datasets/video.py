...
def X2Cube(img, B=[4, 4], skip=[4, 4], n=16):
    M, N = img.shape
    coe, roe = N - B[1] + 1, M - B[0] + 1
    si = np.arange(B[0])[:, None] * N + np.arange(B[1])
    didx = M * N * np.arange(1)
    si = (didx[:, None] + si.ravel()).reshape((-1, B[0], B[1]))
    o_idx = np.arange(roe)[:, None] * N + np.arange(coe)
    dc = np.transpose(np.take(img, si.ravel()[:, None] + o_idx[::skip[0], ::skip[1]].ravel())).reshape(M // 4, N // 4, n)
    return dc
...
class Video(object):
    ...
    def load_img(self):
        if self.imgs is None:
            if iiter in self.img_names[0]:
                self.imgs = [X2Cube(cv2.imread(x, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)) for x in
                             self.img_names]
                self.width = self.imgs[0].shape[1]
                self.height = self.imgs[0].shape[0]
            else:
    ...
    def __iter__(self):
        for i in range(len(self.img_names)):
            if self.imgs is not None:
                ...
                if iiter in self.img_names[i]:
                    img = cv2.imread(self.img_names[i], cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)  # read hsi png
                    img = X2Cube(img)
