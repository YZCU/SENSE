...
def build_opt_lr(model, current_epoch=0):
    if current_epoch >= cfg.BACKBONE.TRAIN_EPOCH:
        ...
        for name, value in model.channelsplit.named_parameters():
            value.requires_grad = True
        for m in model.channelsplit.modules():
            if isinstance(m, nn.BatchNorm2d):
                m.eval()

        for name, value in model.fs.named_parameters():
            value.requires_grad = True
        for m in model.fs.modules():
            if isinstance(m, nn.BatchNorm2d):
                m.eval()
